...
## ADR-009 疑似麻友识别使用独立分析时段和固定圈子共同出现规则

背景：

原疑似麻友识别主要依赖夜间转账、红包/转账备注、自然人排除、单个对手方跨日重复出现。实际调查中，该规则筛出的人员范围偏大，且全局“时段分类”是展示分类概念，不等同于麻友分析窗口。

考虑的选项：

- 维持单个对手方跨日重复出现规则。
- 在原规则上增加金额上限或小额过滤。
- 使用独立分析时段，并以固定圈子中多个重复对手方共同出现作为核心线索。

选定的方案：

使用独立疑似麻友分析时段，默认 20:00 至次日 02:00；在基础筛选后，要求同一晚间 session 内至少配置数量的跨夜重复对手方共同出现。金额不参与命中或排除，只作为统计和排序字段。

理由：

- 独立分析时段更贴近调查业务，可由 GUI 调整，不受全局时段分类影响。
- 固定圈子共同出现比单个对手方重复更能压缩线索范围。
- 麻将资金往来金额可能从小额到数万元，金额过滤会产生错误排除。

后果：

- `detect_mahjong_records()` 输出扩展为明细、对手方统计、圈子统计三类 DataFrame。
- Excel 新增“疑似麻友-圈子统计”工作表。
- CLI 和 GUI 继续通过共享管道使用同一套识别逻辑。
